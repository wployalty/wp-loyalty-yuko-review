...
class Plugin {
	...
	public static function checkDependencies( bool $allow_exit = false ): bool {
		if ( ! self::isPHPCompatible() ) {
			$message = sprintf( '%1$s requires minimum PHP version %2$s', WLYR_PLUGIN_NAME, WLYR_MINIMUM_PHP_VERSION );
		...
		if ( ! self::isWordPressCompatible() ) {
			$message = sprintf( '%1$s requires minimum WordPress version %2$s', WLYR_PLUGIN_NAME, WLYR_MINIMUM_WP_VERSION );
            $allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isActive( 'woocommerce/woocommerce.php' ) ) {
			$message = sprintf( '%1$s requires WooCommerce to be installed and activated in order to be used.', WLYR_PLUGIN_NAME );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isWooCompatible() ) {
			$message = sprintf(  '%1$s requires minimum Woocommerce version %2$s', WLYR_PLUGIN_NAME, WLYR_MINIMUM_WC_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ( ! self::isActive( 'wployalty/wp-loyalty-rules-lite.php' ) ) && ( ! self::isActive( 'wp-loyalty-rules/wp-loyalty-rules.php' ) ) ) {
			$message = sprintf( '%1$s requires WPLoyalty to be installed and activated in order to be used.', WLYR_PLUGIN_NAME );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );

			return false;
		}
		if ( ! self::isLoyaltyCompatible() ) {
			$message = sprintf( '%1$s requires minimum WPLoyalty version %2$s', WLYR_PLUGIN_NAME, WLYR_MINIMUM_WLR_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
	...
	protected static function isPHPCompatible(): bool {
		return (int) version_compare( PHP_VERSION, WLYR_MINIMUM_PHP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isWordPressCompatible(): bool {
		return (int) version_compare( get_bloginfo( 'version' ), WLYR_MINIMUM_WP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isWooCompatible(): bool {
		...
		return (int) version_compare( $woo_version, WLYR_MINIMUM_WC_VERSION, '>=' ) > 0;
	}
	...
	protected static function isLoyaltyCompatible(): bool {
		...
		return (int) version_compare( $wlr_version, WLYR_MINIMUM_WLR_VERSION, '>=' ) > 0;
	}
	...
	protected static function getWLRVersion() {
		...
		return $plugin_folder['wp-loyalty-rules.php']['Version'] ?? '1.0.0';
	}
}
